...
- **Token relay** — the gateway's `BearerTokenRelayInterceptor` is extracted verbatim and contributed as a `RestClientCustomizer`, gated `acme.httpclient.token-relay.enabled=true` **and** `@ConditionalOnClass(JwtAuthenticationToken.class)` so it stays optional and backs off cleanly.
