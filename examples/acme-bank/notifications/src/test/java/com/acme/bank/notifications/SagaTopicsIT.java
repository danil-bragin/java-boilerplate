...
/**
 * BANK-16: proves the auto-configured {@code KafkaAdmin} provisions every saga topic notifications consumes
 * ({@code transfer-completed}, {@code transfer-failed}) as a MULTI-partition topic from
 * {@link com.acme.bank.notifications.config.SagaTopicsConfig} — so notifications never silently subscribes to
 * a 1-partition funnel under {@code allow.auto.create.topics=false}.
 */
...
class SagaTopicsIT {
    ...
    @Test
    void notificationsSagaTopicsAreMultiPartition() {
        List<String> topics = List.of("transfer-completed", "transfer-failed");
